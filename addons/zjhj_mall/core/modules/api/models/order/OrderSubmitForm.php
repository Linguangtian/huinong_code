...
class OrderSubmitForm extends OrderForm
{
    ...
    public function save()
    {
	 ...
	 $order_num_total=count($mchListData);
      if($order_num_total>1){
            $parents_order_no = $this->getOrderNo();
       }
        ...
        foreach ($mchListData as &$mch) {
			...
			if($order_num_total>1){
                $order->parents_order_code=$parents_order_no;
			}
            $order->is_receive_xiaofeigu=0;
